setwd("E:/DarwinFox/KDE/C_N")
require(hypervolume)
require(geometry)

IN_HYPER<-function(samples, all_samples){
  hv = hypervolume(samples, reps=10000, 
                   bandwidth=estimate_bandwidth(samples), quantile=0.5)
...
samples<-read.table("1.txt", head=T, sep=",")
...
write.table(result_a, file="result.1.txt", row.names=F, sep=",")